...
implements Listener {
    private static final Map<Material, Double> DAMAGE = new HashMap<Material, Double>();
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        Bukkit.getPluginManager().callEvent((Event)playerDamagePlayerEvent);
    ...
    public void onEntityDamageEvent(EntityDamageByEntityEvent event) {
        ...
        if (p.hasPotionEffect(PotionEffectType.INCREASE_DAMAGE)) {
            for (PotionEffect effect : p.getActivePotionEffects()) {
                if (!effect.getType().equals((Object)PotionEffectType.INCREASE_DAMAGE)) continue;
                double minus = this.isCrital(p) ? (danoEspada + danoEspada / 2.0) * 1.3 * (double)(effect.getAmplifier() + 1) : danoEspada * 1.3 * (double)(effect.getAmplifier() + 1);
                damage -= minus;
