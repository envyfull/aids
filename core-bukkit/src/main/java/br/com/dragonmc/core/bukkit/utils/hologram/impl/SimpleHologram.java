 ...
 */
package br.com.dragonmc.core.bukkit.utils.hologram.impl;
...
import br.com.dragonmc.core.bukkit.utils.hologram.TouchHandler;
import br.com.dragonmc.core.bukkit.utils.hologram.ViewHandler;
...
implements Hologram {
    ...
    public SimpleHologram(String displayName, Location location, TouchHandler touchHandler, ViewHandler viewHandler) {
        ...
        this.hologramLines = new ArrayList<Hologram>();
        this.invisibleTo = new ArrayList<Player>();
    }
    ...
    public Collection<? extends Player> getViewers() {
        Collection clone = Bukkit.getOnlinePlayers();
    ...
    private ArmorStand createArmorStand() throws Exception {
        ...
        for (Entity entity : this.location.getWorld().getNearbyEntities(this.location, 0.3, 0.3, 0.3)) {
            if (!(entity instanceof ArmorStand) || entity.isCustomNameVisible() != this.isCustomNameVisible() || !entity.getCustomName().equals(this.displayName)) continue;
    ...
    public boolean isEntityOrLine(int entityId) {
        ...
        if (this.armorStand.getEntityId() == entityId) {
            return true;
        }
        return this.hologramLines.stream().filter(hologram -> this.getEntityId() == entityId).findFirst().isPresent();
    }
