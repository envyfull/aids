...
public class MemberManager {
    private Map<UUID, Member> memberMap = new HashMap<UUID, Member>();
    ...
    public <T extends Member> T getMember(UUID uniqueId, Class<T> clazz) {
        if (this.memberMap.containsKey(uniqueId)) {
            return (T)((Member)clazz.cast(this.memberMap.get(uniqueId)));
        }
        return null;
    }
    ...
    public <T extends Member> T getMemberByName(String playerName, Class<T> clazz) {
        ...
        return (T)(orElse == null ? null : (Member)clazz.cast(orElse));
    }
    ...
    public <T extends Member> Collection<T> getMembers(Class<T> clazz) {
        return (Collection<T>) this.getMembers().stream().filter(member -> clazz.isAssignableFrom(member.getClass())).map(member -> (Member)clazz.cast(member)).collect(Collectors.toList());
    }
    ...
    public void staffLog(String message, boolean format) {
        this.getMembers().stream().filter(member -> member.hasPermission("staff.log") && member.getMemberConfiguration().isSeeingLogs()).forEach(member -> member.sendMessage(format ? "\u00a77[" + message + "\u00a77]" : message));
        System.out.println(message);
    }

    public void staffLog(TextComponent textComponent) {
        this.getMembers().stream().filter(member -> member.hasPermission("staff.log") && member.getMemberConfiguration().isSeeingLogs()).forEach(member -> member.sendMessage((BaseComponent)textComponent));
