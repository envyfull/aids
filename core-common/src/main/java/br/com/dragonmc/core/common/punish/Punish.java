...
public class Punish {
    ...
    public Punish(UUID playerId, String playerName, UUID punisherId, String punisherName, String punishReason, long expireAt, PunishType punishType) {
        this.id = "#" + Punish.randomId(punishType);
    ...
    public String getMuteMessage(Language language) {
        return language.t("mute-message", "%reason%", this.getPunishReason(), "%expireAt%", DateUtils.getTime(language, this.getExpireAt()), "%punisher%", this.getPunisherName(), "%website%", CommonPlugin.getInstance().getPluginInfo().getWebsite(), "%store%", CommonPlugin.getInstance().getPluginInfo().getStore(), "%discord%", CommonPlugin.getInstance().getPluginInfo().getDiscord());
    }
    ...
    public static String randomId(PunishType punishType) {
        ...
        } while (((MongoQuery)CommonPlugin.getInstance().getMemberData().getQuery()).getCollection().find(Filters.eq((String)("punishConfiguration.punishMap." + punishType.name() + ".id"), (Object)id)) == null);
        return id;
    }
