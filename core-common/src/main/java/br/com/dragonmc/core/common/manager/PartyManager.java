...
public class PartyManager {
    ...
    private Map<UUID, Map<UUID, InviteInfo>> partyInvitesMap = new HashMap<UUID, Map<UUID, InviteInfo>>();
    ...
    public Party getPartyByOwner(UUID uniqueId) {
        return this.partyMap.values().stream().filter(party -> party.getMembersMap().containsKey(uniqueId) && party.getMembersMap().get(uniqueId) == PartyRole.OWNER).findFirst().orElse(null);
    }
